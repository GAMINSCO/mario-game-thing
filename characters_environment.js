...
function game() {
  ...
  if (gameConfig.status === 'start') {
    ...
    textSize(40);
    textAlign(CENTER);
    text("Press the start button to start playing! ", gameConfig.screenX / 2, gameConfig.screenY / 2);
    textSize(40);
...
function initializeCharacterStatus(character) {
  ...
  character["kills"] = 0;
  character["live"] = true;
  character["liveNumber"] = gameConfig.initialLives;
...
/* See if  obj1 stand on obj2, mainly for see if standing on the objects*/
...
function doNotGetOutOfScreen(character) {
...
function scores(character) {
  ...
  if (mario.live == false && mario.liveNumber != 0) {
    ...
    stroke("red");
    var ratio = (character.liveNumber / gameConfig.initialLives);
